...
import numpy as np
def text_split1(sentence):
    kkma = Kkma()
    return (kkma.morphs(sentence))

def text_split2(sentence):
    kkma = Kkma()
    return (kkma.nouns(sentence))

def text_split3(sentence):
    kkma = Kkma()
    return (kkma.pos(sentence))
...
def find_problem(text):
    ...
    # text2=""
    # text3=""
    # text1 = text_split1(text)
    # text2= text_split2(text)
    # text3= text_split3(text)
    # text = text1+text2+text3
    problem = ["default","love","relation","health_","study","wealth","job"]
    problem_list = [0,0,0,0,0,0,0]
    default = ["몰라","없어"]
    love = ["연애","남자친구","여자친구","사랑","남친","여친","자기","썸","영화","밥","남사친","여사친","재회","새로운","이별","헤어짐","바람","연애","남자","여자"]
    relation = ["친구","만남","주변인","지인","사람","손절","주변 사람","친우","대화","학교","연락","대학","동네친구","배려","예의","선","베프","친한","인스타","카톡"]
    health = ["건강","병원","만수무강","질병","부상","회복","나이","피곤해","졸려","잠","지쳤다","눈","피곤","피곤하다","졸립다","잠온다","졸리"]
    study = ["공부","학업","문제","등수","전교","등급","과목","학점","머리","두뇌","수학","영어","국어","필기","시험","장학금","입시","졸전","전시회","졸업"]
